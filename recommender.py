...
class Recommender:
    ...
    def recommend_news_based_on_keywords_and_preferences(self):
        ...
        if self.user_categories:
            category_weights = self.user_categories
            category_max_score = max(category_weights.values(), key=lambda x: x['score'])['score']
            for category, weight_info in category_weights.items():
                weight = weight_info['score']
                self.data.loc[self.data['category'] == category, 'combined_similarity'] *= (weight / category_max_score)
    ...
    def recommend_news_based_on_keywords_and_preferences(self):
        ...
        for keyword, score_info in keywords.items():
            ...
            # Calculate cosine similarity for each feature with the keyword
            data['title_similarity'] = data.apply(lambda row: cosine_similarity([row['title_vector']], [keyword_vec])[0][0], axis=1)
            data['short_desc_similarity'] = data.apply(lambda row: cosine_similarity([row['short_desc_vector']], [keyword_vec])[0][0], axis=1)
            data['desc_similarity'] = data.apply(lambda row: cosine_similarity([row['desc_vector']], [keyword_vec])[0][0], axis=1)
        ...
        # Remove unnecessary columns
        data = data.drop(columns=['title_vector', 'short_desc_vector', 'desc_vector', 'title_similarity',
                                            'short_desc_similarity', 'desc_similarity'])
...
############################################################################################################
    ...
    def preprocessing(self, text):
        ...
        text = self.make_lower_case(text)
    ...
    def update_dislike_keywords(self, text, current_time, weight=0.1):
        ...
        for keyword in text:
            if keyword in self.user_keywords:
                ...
                self.user_keywords[keyword] = {'score': 0.5, 'last_modified': current_time}
        return self.user_keywords
    ...
    def update_dislike_categories(self, category, current_time, weight=0.1):
        if category in self.user_categories:
            ...
            self.user_categories[category] = {'score': 0.5, 'last_modified': current_time}
        return self.user_categories
    ...
    def update_view_history(self, article_history):
        ...
        for article in article_history:
            ...
            # Update keywords
            self.update_like_keywords(article['title'], self.user_keywords, current_time, weight=0.1)

            # Update categories
            self.update_like_categories(category, self.user_categories, current_time, weight=0.1)
        ...
        updated_keywords = self.delete_overflow_keywords(updated_keywords)
        updated_categories = self.delete_overflow_categories(updated_categories)
    ...
    def set_feedback(self, article, feedback):
        ...
        if feedback.lower() == 'like':
            if article not in self.user_likes:
                self.user_keywords = self.update_like_keywords(article['title'], self.user_keywords, current_time, weight=0.8)
        elif feedback.lower() == 'dislike':
            if article not in self.user_dislikes:
                self.user_keywords = self.update_dislike_keywords(article['title'], self.user_keywords, current_time, weight=0.8)

        # Update category
        if feedback.lower() == 'like':
            if article not in self.user_likes:
                self.user_categories = self.update_like_categories(article['category'], self.user_categories, current_time, weight=0.8)
        elif feedback.lower() == 'dislike':
            if article not in self.user_dislikes:
                self.user_categories = self.update_dislike_categories(article['category'], self.user_categories, current_time, weight=0.8)
